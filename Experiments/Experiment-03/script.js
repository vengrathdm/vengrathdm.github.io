// Experiment 03 — rozszerzone archiwum wróżenia z kryształowej kuli
...
const visions = [
  {
    ...
    html:`<div class="page">
      <small>KLĄTWA ŚMIERCI · AKTUALNY ZAPIS</small>
  ...
  },
  {
    ...
    html:`<div class="page">
      ...
      <div class="list">
        ...
        <div><span>DZIEWIĘCIU BOGÓW</span><b>Drużyna wchodzi do Grobowca.</b></div>
  ...
  },
  {
    ...
    html:`<div class="page">
      ...
      <div class="grid">
        ...
        <article><b>DŻUNGLA</b><p>Rzeki, urwiska, ruiny, patrole nieumarłych i stworzenia dość wielkie, by pożreć drużynę.</p></article>
        <article><b>OMU</b><p>Martwe święte miasto, w którym drużyna rozwiązała zagadki świątyń i odnalazła Grobowiec.</p></article>
        <article><b>GROBOWIEC</b><p>Śmiertelna pułapka zbudowana wokół duchów Dziewięciu Podstępnych Bogów i Soulmongera.</p></article>
      </div>
    </div>`
  },
  {
    ...
    html:`<div class="page">
      ...
      <h1>PIĘCIORO<br><em>KTÓRZY WESZLI</em></h1>
      <div class="list">
...
function scryTransition(){
  crystal.classList.remove("scrying");
  void crystal.offsetWidth;
  crystal.classList.add("scrying");
  clearTimeout(scryingTimer);
  scryingTimer=setTimeout(()=>crystal.classList.remove("scrying"),1500);
}
...
function openVision(index){
  ...
  omenText.textContent=currentVision===6?"SOULMONGER JEST GŁODNY.":currentVision===8?"DRZWI JUŻ SIĘ OTWORZYŁY.":"UMARLI NIE ŚPIĄ.";
  ...
  setTimeout(()=>{vision.innerHTML=item.html;},560);
}

function toggleAwaken(){
  awakened=!awakened;
  crystal.classList.toggle("sealed",!awakened);
  document.getElementById("awaken").textContent=awakened?"ZAPIECZĘTUJ WIZJĘ":"OŻYW WIZJĘ";
}
...
window.addEventListener("pointermove",e=>{
  ...
  document.documentElement.style.setProperty("--mx",`${pointer.x*2}`);
  document.documentElement.style.setProperty("--my",`${pointer.y*2}`);
});

const observer=new MutationObserver(()=>{
  if(currentVision===7){
    const img=document.querySelector(".vision-image.jungle");
    if(img) img.classList.add("living");
  }
  if(currentVision===8){
    const img=document.querySelector(".vision-image.tomb");
    if(img) setTimeout(()=>img.classList.add("opening"),700);
  }
});
observer.observe(vision,{childList:true});