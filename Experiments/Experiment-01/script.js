// Experiment 01 — Tomb of Annihilation campaign archive
...
function togglePower() {
  ...
  document.getElementById("power").textContent = isPoweredOn ? "POWER" : "POWER ON";
}
...
document.querySelectorAll(".tapes button").forEach(button => {
  button.addEventListener("click", () => {
    ...
    tapeStatus.textContent = "PLAYING / " + title.toUpperCase();
...
document.getElementById("ff").addEventListener("click", () => {
  tapeStatus.textContent = "FAST FORWARD";
  setReelDirection("normal");
  setReelSpeed(".45s");
});

document.getElementById("eject").addEventListener("click", () => {
  tapeStatus.textContent = "NO TAPE";
  cassetteLabel.textContent = "SELECT A CAMPAIGN TAPE";
  setReelState("paused");
  document.querySelectorAll(".tapes button").forEach(item => item.classList.remove("active"));
});
