...
function build(){const H=Math.max(420,viewport.clientHeight||innerHeight-186),W=Math.max(viewport.clientWidth||innerWidth,320),cur=current(),rows=5,tileW=Math.max(180,Math.min(276,W/5.4)),minCols=Math.max(1,Math.ceil(W/tileW)),fallback=cur.length>0&&cur.length<minCols*3;let worldW=W,sites=[];
