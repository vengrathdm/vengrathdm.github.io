...
sites.forEach((s,i)=>{const raw=cell(s,sites,{x0:0,y0:0,x1:worldW,y1:H});if(raw.length<3)return;const poly=shape(raw,i,worldW,H);const cp=document.createElementNS("http://www.w3.org/2000/svg","clipPath"),id="clip"+i;cp.id=id;const cpPath=document.createElementNS("http://www.w3.org/2000/svg","path");cpPath.setAttribute("d",path(poly));cp.appendChild(cpPath);defs.appendChild(cp);const a=document.createElementNS("http://www.w3.org/2000/svg","a");a.classList.add("shard");a.setAttribute("href",s.data.link);a.setAttribute("tabindex","0");a.setAttribute("aria-label",s.data.title);a.addEventListener("click",e=>{e.preventDefault()});const xs=poly.map(p=>p.x),ys=poly.map(p=>p.y),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys);const im=document.createElementNS("http://www.w3.org/2000/svg","image");im.setAttribute("x",minX);im.setAttribute("y",minY);im.setAttribute("width",maxX-minX);im.setAttribute("height",maxY-minY);im.setAttribute("preserveAspectRatio","xMidYMid slice");im.setAttribute("clip-path",`url(#${id})`);im.dataset.src=s.data.graphic;im.classList.add("shard-image");a.appendChild(im);const tint=document.createElementNS("http://www.w3.org/2000/svg","path");tint.setAttribute("d",path(poly));tint.classList.add("shard-tint");a.appendChild(tint);const g=document.createElementNS("http://www.w3.org/2000/svg","g");g.setAttribute("clip-path",`url(#${id})`);addText(g,s,poly);a.appendChild(g);const flash=document.createElementNS("http://www.w3.org/2000/svg","rect");flash.setAttribute("x",minX-60);flash.setAttribute("y",minY);flash.setAttribute("width",34);flash.setAttribute("height",maxY-minY);flash.setAttribute("clip-path",`url(#${id})`);flash.classList.add("shard-flash");a.appendChild(flash);a.addEventListener("pointerenter",()=>{flash.classList.remove("play");void flash.getBoundingClientRect().width;flash.classList.add("play")});svg.appendChild(a)});board.appendChild(svg);lazyLoadImages()}
...
function release(e){if(!drag.active)return;const moved=drag.moved;const target=e.target&&e.target.closest?e.target.closest(".shard"):null;const link=target&&target.getAttribute("href");drag.active=false;viewport.style.cursor="grab";try{viewport.releasePointerCapture(e.pointerId)}catch(_){}let v=drag.velocity;const tick=()=>{viewport.scrollLeft+=v;v*=.94;if(Math.abs(v)>.15)momentum=requestAnimationFrame(tick)};if(Math.abs(v)>.15)momentum=requestAnimationFrame(tick);if(!moved&&link)window.location.assign(link);setTimeout(()=>drag.moved=false,100)}
